...
def init_driver(status_callback=None):
    ...
    )  # Go up from src/backend to main folder
    ...
    if os.path.exists(driver_path):
        if status_callback:
            status_callback("Local driver found. Starting...")
        ...
        try:
            ...
            raise Exception(
                f"Download 'msedgedriver.exe' and put it in the AutoSender folder. Error details: {e2!s}"
            )
...
def send_whatsapp_message(
    contact_name, message, status_callback=None, existing_driver=None, keep_open=False
):
    """
    ...
    try:
        if not driver:
            ...
            try:
                ...
                if status_callback:
                    status_callback(
                        "Waiting for WhatsApp Web (scan the QR code if requested)..."
                    )
        ...
        if existing_driver:
            ...
            try:
                ...
                if normalized_contact and (
                    normalized_contact in header_text
                    or header_text in normalized_contact
                ):
                    already_in_chat = True
                    if status_callback:
                        status_callback(
                            "Already inside the chat! Skipping search phase."
                        )
            except Exception:
                pass
        ...
        if not already_in_chat:
            ...
            try:
                ...
                search_box = WebDriverWait(driver, 45).until(
            ...
            if not search_box:
                raise Exception(
                    "Cannot find the search bar. WhatsApp Web might have changed."
                )
            ...
            try:
                ...
                webdriver.ActionChains(driver).move_to_element(
                    search_box
                ).click().send_keys(contact_name).perform()
            ...
            try:
                contact_el = WebDriverWait(driver, 5).until(
        ...
        if not message_box:
            ...
            raise Exception(
                "Cannot find the message bar. Maybe the chat didn't open correctly?"
            )
        ...
        try:
            ...
            raise Exception(f"Error during final message sending: {e!s}")
        ...
        if status_callback:
            status_callback(f"Critical Error: {e!s}")
            status_callback(
                "The window will stay open for 20 seconds to show you what went wrong..."
            )
