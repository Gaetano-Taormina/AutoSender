    ...
    while True:
        try:
            ...
            if parent_pid:
                ...
                if process == 0:
                    write_log(
                        "Process monitor: Parent process died. Shutting down scheduler."
                    )
                    if scheduler_driver:
                        try:
            ...
            for task in tasks:
                # If passed more than 5 minutes (300 sec), put it in the trash
                if now > task["timestamp"] + 300:
                    write_log(
                        f"Message for {task['contact']} expired. Moving to trash."
                    )
                    trash_missed_message(task["contact"], task["message"])
                    mark_task_done(
                        task["id"], success=False, error="Missed scheduled time"
                    )
            ...
            if due_tasks:
                for task in due_tasks:
                    ...
                    # Real-time check: was the task cancelled by user while waiting?
                    current_pending = get_pending_tasks()
                    if not any(t["id"] == task_id for t in current_pending):
                        write_log(
                            f"Task for {contact} was cancelled by user. Skipping."
                        )
                        continue
                    ...
                    try:
                        result = send_whatsapp_message(
                            contact,
                            message,
                            status_callback=write_log,
                            existing_driver=scheduler_driver,
                            keep_open=True,
                        )
                        ...
                        write_log(
                            f"Critical exception during send to {contact}: {e!s}"
                        )
            ...
            if future_tasks:
                ...
                if time_to_next > 180 and scheduler_driver:
                    write_log(
                        f"Next message is in {int(time_to_next)} seconds. Closing Edge for now."
                    )
                    try:
                        scheduler_driver.quit()
                    except Exception:
...
def signal_handler(signum, frame):
    write_log(
        f"Process monitor: Received termination signal ({signum}). Shutting down scheduler."
    )
    sys.exit(0)
